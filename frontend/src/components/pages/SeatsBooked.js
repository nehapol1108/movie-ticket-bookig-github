    ...
    try{
      ...
      data.data.data.seatbooked.map(async(ele)=>{
        if((ele.booked.length) >0 ){
           const movieData = await axios.get(`/api/movie/${ele.movieId}`,config);
              ...
              <div className="preedit"><span className="postedit">Movie Time : </span>{movie?.movietime}</div>
                ...
                {seatBooked.map((seat)=>{
                  if(seat.movieId === movie.movieId){
                    return(
                      <>
                      <span className="postedit"> ------------Seats Booked------------ <div className="preedit">
                      {seat.booked.map((ele)=>{
                        return(
                          <>    {ele}</>
                  ...
                  <span className="postedit">------------------------------------------</span>
              <div className="preedit"><span className="postedit">Total Price : </span>{seat.booked.length* parseInt(movie.movietprice)}</div>
